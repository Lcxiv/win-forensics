...
Required: `time_of_day` (verbatim), `process_name`, `pid`, `operation`, `path`, `result`, `detail`, `provenance` (`row:<n>`). Optional, present only when the capture configuration selected the column: `sequence`, `date_and_time` (verbatim), `relative_time` (verbatim), `duration_s`, `tid`, `parent_pid`, `image_path`, `command_line`, `company`, `description`, `version`, `user`, `session`, `authentication_id`, `integrity`, `architecture`, `virtualized`, `category`, `event_class`, `completion_time` (verbatim). Derived: `time_utc` (ISO 8601 UTC computed from `time_of_day`, the capture date from the sidecar, and the manifest time zone; null when the export did not carry enough precision or the date is ambiguous). Stacks never appear in a CSV export; a `.pml` decoder in a later phase adds a `procmon_stacks` table.
